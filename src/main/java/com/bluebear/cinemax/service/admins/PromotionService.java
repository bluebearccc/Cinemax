...
public class PromotionService {
    ...
    // Sửa method này để nhận Promotion_Status thay vì String
    private boolean isValidStatus(Promotion_Status status) {
        return status != null && 
                (status == Promotion_Status.Available || status == Promotion_Status.Expired);
    }
    ...
    private void updateVoucherFields(Promotion voucher, PromotionDTO dto) {
        ...
        // Sửa dòng này - thêm setStatus method call
        Optional.ofNullable(dto.getStatus()).ifPresent(voucher::setStatus);
    }
